...
import type {
  Feedback, NewsItem, PersonalReference, PhraseCard, Project, ProjectFile, QuickSession, Segment, SkillManifest, StyleGuide, TermEntry,
    ...
    <div className="v2-shell">
      ...
      <main className="v2-main">
        ...
        <div className="v2-page">
          ...
          {activeView === 'news' && <NewsFeed workspace={workspace} onWorkspace={patch} onPractice={(news, excerpt, paragraphId) => { const now = new Date().toISOString(); const exercise = { id: createId('practice'), contentId: news.id, sourceUrl: news.url, sourceText: excerpt, selectedExcerpt: excerpt, sourceParagraphId: paragraphId, sourceLanguage: news.language, targetLanguage: news.language === '英语' ? '简体中文（zh-CN）' : '英语（en）', domain: news.category, style: '自然', audience: '普通读者', mode: 'student-first' as const, studentDraft: '', aiPretranslation: '', aiReference: '', finalTranslation: '', feedback: [], createdAt: now, updatedAt: now }; patch((current) => ({ ...current, practices: [exercise, ...current.practices] })); setQuick({ ...emptyQuick(), source: excerpt, detectedLanguage: news.language, domain: news.category, textType: news.contentType, audience: '普通读者', mode: 'student-first' }); navigate('quick'); setToast('选段已建立为翻译练习，并保留文章来源') }} />}
          ...
          {activeView === 'skills' && <SkillCenter workspace={workspace} onWorkspace={patch} onToast={setToast} />}
...
function Projects({ workspace, activeProject, initialMode, onSelectProject, onWorkspace, onCreate, onToast }: { workspace: Workspace; activeProject?: Project; initialMode: 'list' | 'editor'; onSelectProject: (id: string) => void; onWorkspace: (fn: (current: Workspace) => Workspace) => void; onCreate: (project: Project) => void; onToast: (message: string) => void }) {
  ...
  const visibleProjects = workspace.projects.filter((project) => (status === 'all' || project.status === status) && `${project.name}${project.description}${project.domain}${project.files.map((file) => file.name).join('')}`.toLowerCase().includes(query.trim().toLowerCase()))
  ...
  return <div className="v2-stack"><PageHead eyebrow="PROJECTS" title="翻译项目" text="长文本和多文件任务在片段、上下文、语言资产与历史版本中推进。" actions={<button className="v2-btn primary" onClick={() => setMode('create')}><Plus size={16} />创建项目</button>} /><section className="project-toolbar"><div><Search size={16} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="搜索项目名称、领域或文件……" /></div><label><SlidersHorizontal size={15} /><select value={status} onChange={(event) => setStatus(event.target.value as typeof status)}><option value="all">全部状态</option>{(['draft','active','paused','review','completed','archived'] as const).map((item) => <option key={item} value={item}>{item}</option>)}</select></label></section><section className="project-table"><header><span>项目</span><span>文件</span><span>片段进度</span><span>质量问题</span><span>最近修改</span><span /></header>{visibleProjects.map((project) => { const issues = workspace.qualityIssues.filter((item) => projectSegments(project).some((segment) => segment.id === item.segmentId) && !item.resolved).length; return <button className="project-row" key={project.id} onClick={() => open(project.id)}><span><i>{project.status}</i><strong>{project.name}</strong><small>{project.domain} · {project.sourceLanguage} → {project.targetLanguage}</small></span><b>{project.files.length}</b><span><em><i style={{ width: `${projectProgress(project)}%` }} /></em><small>{projectProgress(project)}%</small></span><b className={issues ? 'warn' : ''}>{issues || '—'}</b><small>{dateTime(project.updatedAt)}</small><ChevronRight size={17} /></button> })}{!visibleProjects.length && <Empty title="没有符合条件的项目" text="调整关键词或状态筛选，或创建一个新项目。" />}</section></div>
}
...
function ProjectEditor({ project, workspace, onBack, onWorkspace, onToast }: { project: Project; workspace: Workspace; onBack: () => void; onWorkspace: (fn: (current: Workspace) => Workspace) => void; onToast: (message: string) => void }) {
  ...
  const [rightTab, setRightTab] = useState<'pretranslation' | 'ai' | 'tm' | 'terms' | 'culture' | 'context' | 'quality' | 'history' | 'notes'>('pretranslation')
  ...
  const runLookup = async () => {
    ...
    setLookingUp(true)
    try { const result = await lookupTerm(lookupText, active, project, workspace.terms, workspace.settings); updateSegment(active.id, { termLookups: [result, ...active.termLookups.filter((item) => item.term.toLowerCase() !== result.term.toLowerCase())] }); onToast(result.provider === 'local' ? '已从个人术语库找到结果' : '术语解释与来源已保存') } catch (error) { onToast(error instanceof Error ? error.message : '术语查询失败') } finally { setLookingUp(false) }
  }
  const runBatch = async (scope: 'project' | 'file' | 'selection', existing = activeJob) => {
    ...
    stopBatch.current = ''; setBatchRunning(true)
    ...
    if (stopBatch.current && !['completed', 'failed', 'cancelled'].includes(job.status)) {
      job = { ...job, status: stopBatch.current === 'cancel' ? 'cancelled' : 'paused', updatedAt: new Date().toISOString() }
      onWorkspace((current) => ({ ...current, batchJobs: current.batchJobs.map((item) => item.id === job.id ? job : item) }))
    }
    setBatchRunning(false)
    onToast(job.status === 'completed' ? '批量预翻译已完成，所有结果仍需人工确认' : job.status === 'failed' ? `任务完成，但有 ${job.failedSegmentIds.length} 个片段失败` : job.status === 'cancelled' ? '批量任务已取消，已完成的片段仍保留' : '批量任务已暂停，可随时继续')
  }
  ...
  return <div className="editor-page"><header className="editor-toolbar"><button className="icon-button" onClick={onBack}>←</button><div><strong>{project.name}</strong><small>{file.name}</small></div><div className="editor-progress"><span>{projectProgress(project)}%</span><i><b style={{ width: `${projectProgress(project)}%` }} /></i></div><label className="editor-search"><Search size={14} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="搜索原文或译文" /></label><button onClick={runQuality}><ShieldCheck size={15} />质量检查</button><button onClick={exportPure}><Download size={15} />纯译文</button><button onClick={exportBilingual}><FileCheck2 size={15} />双语文档</button><span className="saved-label"><Check size={13} />自动保存</span><button className="icon-button" onClick={() => setRightOpen(!rightOpen)}>{rightOpen ? <PanelRightClose size={17} /> : <PanelRightOpen size={17} />}</button></header>
    ...
    <div className={rightOpen ? 'editor-shell' : 'editor-shell right-closed'}><aside className="file-rail"><span>项目文件</span>{project.files.map((item) => <button key={item.id} className={item.id === file.id ? 'active' : ''} onClick={() => { setActiveFileId(item.id); setActiveSegmentId(item.segments[0]?.id ?? '') }}><FileText size={15} /><div><strong>{item.name}</strong><small>{item.segments.length}个片段</small></div></button>)}<span>片段状态</span>{([['all','全部'],['untranslated','未翻译'],['needs-confirmation','待确认'],['confirmed','已确认'],['issue','有问题']] as const).map(([value,label]) => <button key={value} className={filter === value ? 'active' : ''} onClick={() => setFilter(value)}><Circle size={12} /><div><strong>{label}</strong><small>{value === 'all' ? file.segments.length : file.segments.filter((item) => item.status === value).length}</small></div></button>)}</aside>
      ...
      {rightOpen && <aside className="editor-assist"><nav>{([['pretranslation','AI预译文'],['ai','修改建议'],['terms','术语解释'],['culture','文化背景'],['tm','翻译记忆'],['context','上下文'],['quality','质量问题'],['history','修改记录'],['notes','批注笔记']] as const).map(([value,label]) => <button key={value} className={rightTab === value ? 'active' : ''} onClick={() => setRightTab(value)}>{label}</button>)}</nav><div>
        {rightTab === 'pretranslation' && <>{active.aiPretranslation ? <div className="ai-reference"><span>AI原始预译文 · 不随人工修改覆盖</span><p>{active.aiPretranslation}</p><div><button onClick={() => navigator.clipboard?.writeText(active.aiPretranslation).then(() => onToast('AI预译文已复制'))}><Copy size={13} />只复制</button><button onClick={() => applyAIReference(active)}><WandSparkles size={13} />应用为当前译文</button></div></div> : <Empty title="当前片段没有AI预译文" text="运行批量预翻译，或在“修改建议”中请求片段级参考。" />}</>}
        ...
        {rightTab === 'terms' && <><div className="term-lookup"><label>查询当前片段中的词或短语<input value={lookupText} onChange={(event) => setLookupText(event.target.value)} placeholder="例如：human agency" /></label><button disabled={!lookupText.trim() || lookingUp} onClick={() => void runLookup()}>{lookingUp ? <LoaderCircle className="spin" size={13} /> : <Search size={13} />}查询术语</button></div>{active.termLookups.map((result) => <div className="lookup-result" key={result.id}><span>{result.provider === 'local' ? '个人术语库' : '安全代理检索'}</span><h3>{result.term} <small>{result.partOfSpeech}</small></h3><p>{result.contextMeaning || result.meaning}</p><strong>{result.recommendedTranslations.join('／') || '暂无推荐译法'}</strong>{result.forbiddenTranslations.length > 0 && <small>不推荐：{result.forbiddenTranslations.join('、')}</small>}<div>{result.sources.map((source,index) => source.url ? <a key={`${source.title}-${index}`} href={source.url} target="_blank" rel="noreferrer">{source.title}</a> : <i key={`${source.title}-${index}`}>{source.title}</i>)}</div><button onClick={() => { const entry: TermEntry = { id: createId('term'), source: result.term, target: result.recommendedTranslations[0] ?? '', definition: result.contextMeaning || result.meaning, domain: project.domain, partOfSpeech: result.partOfSpeech, allowed: result.recommendedTranslations, forbidden: result.forbiddenTranslations, example: result.examples[0] ?? '', sourceRef: result.sources.map((source) => source.title).join('；'), status: 'candidate' }; onWorkspace((current) => ({ ...current, terms: current.terms.some((term) => term.source.toLowerCase() === entry.source.toLowerCase() && term.domain === entry.domain) ? current.terms : [entry, ...current.terms] })); onToast('已加入候选术语，需人工审核') }}>加入术语库</button></div>)}{workspace.terms.filter((term) => active.source.toLowerCase().includes(term.source.toLowerCase())).map((term) => <div className="term-detail" key={term.id}><span>{term.status}</span><h3>{term.source}</h3><strong>{term.target || '待核验'}</strong><p>{term.definition}</p><small>{term.sourceRef}</small></div>)}</>}
        {rightTab === 'culture' && (active.culturalNotes.length ? active.culturalNotes.map((note) => <div className="lookup-result" key={note.id}><span>{note.category} · {note.provider}</span><h3>{note.expression}</h3><p>{note.explanation}</p><strong>{note.translationAdvice}</strong><div>{note.sources.map((source,index) => source.url ? <a key={`${source.title}-${index}`} href={source.url} target="_blank" rel="noreferrer">{source.title}</a> : <i key={`${source.title}-${index}`}>{source.title}</i>)}</div></div>) : <Empty title="暂无可核验的文化背景" text="文化概念识别依赖安全代理与来源检索；当前不会用无来源的模型解释填充此处。" />)}
...
function NewsFeed({ workspace, onWorkspace, onPractice }: { workspace: Workspace; onWorkspace: (fn: (current: Workspace) => Workspace) => void; onPractice: (item: NewsItem, excerpt: string, paragraphId: string) => void }) {
  ...
  const categories = ['全部', ...Array.from(new Set(items.map((item) => item.category))), '我的收藏', '已读']
  const visible = items.filter((item) => filter === '全部' || filter === '我的收藏' ? (filter === '全部' || item.saved) : filter === '已读' ? item.read : item.category === filter)
  const reader = items.find((item) => item.id === readerId)
  const changeItem = (id: string, patch: Partial<NewsItem>) => onWorkspace((current) => ({ ...current, news: current.news.map((item) => item.id === id ? { ...item, ...patch, updatedAt: new Date().toISOString() } : item) }))
  const sync = async () => { setSyncing(true); setSyncMessage('正在通过内容代理同步…'); const report = await syncContentSources(workspace); onWorkspace((current) => ({ ...current, news: report.items, sources: report.sources })); setSyncMessage(report.error || `新增 ${report.added} · 更新 ${report.updated} · 去重 ${report.deduplicated} · 失败 ${report.failed}`); setSyncing(false) }
  const practice = (item: NewsItem, excerpt: string, paragraphId: string) => { if (!excerpt.trim()) { setSyncMessage('请先粘贴或选择一个可合法使用的短片段'); return } onPractice(item, excerpt.trim(), paragraphId); setReaderId(''); setCustomExcerpt('') }
  return <div className="v2-stack"><PageHead eyebrow="READ · VERIFY · TRANSLATE" title="译闻与素材" text="按来源与版权边界保存内容；实时同步只经过你配置的安全代理。" actions={<button className="v2-btn subtle" disabled={syncing} onClick={() => void sync()}>{syncing ? <LoaderCircle className="spin" size={16} /> : <RefreshCcw size={16} />}{syncing ? '正在同步' : '刷新来源'}</button>} />{syncMessage && <div className={syncMessage.includes('失败') || syncMessage.includes('未配置') ? 'sync-status error' : 'sync-status'}>{syncMessage}</div>}<section className="news-toolbar"><div>{categories.map((item) => <button className={filter === item ? 'active' : ''} key={item} onClick={() => setFilter(item)}>{item}</button>)}</div><span><ShieldCheck size={15} />{items.length}条可追溯内容记录</span></section><div className="news-layout"><aside><strong>来源状态</strong>{workspace.sources.map((source) => <div key={source.id}>{source.lastError ? <WifiOff size={14} /> : <Wifi size={14} />}<span>{source.name}</span><small>{source.enabled ? source.lastError || (source.lastSyncedAt ? dateTime(source.lastSyncedAt) : '待首次同步') : '未启用'}</small><button onClick={() => onWorkspace((current) => ({ ...current, sources: current.sources.map((item) => item.id === source.id ? { ...item, enabled: !item.enabled, updatedAt: new Date().toISOString() } : item) }))}>{source.enabled ? '停用' : '启用'}</button></div>)}<p>元数据型内容不保存全文；开放许可、公共领域或用户自有文本才可在站内显示正文。</p></aside><section>{visible.map((item) => <article className="news-item" key={item.id}><header><span>{item.source}</span><time>{item.publishedAt}</time><button aria-label={item.saved ? '取消收藏' : '收藏'} onClick={() => changeItem(item.id, { saved: !item.saved })}>{item.saved ? <BookMarked size={16} /> : <BookOpen size={16} />}</button></header><h2>{item.title}</h2><p>{item.summary}</p><div><span>{item.category}</span>{item.tags.map((tag) => <span key={tag}>{tag}</span>)}<i>{item.level} · {item.readingMinutes}分钟</i></div><blockquote>{item.trainingDirection}</blockquote><footer><button onClick={() => { setReaderId(item.id); changeItem(item.id, { read: true, readingProgress: Math.max(item.readingProgress, 15) }) }}>站内阅读 <Eye size={14} /></button><button onClick={() => changeItem(item.id, { read: !item.read, readingProgress: item.read ? 0 : 100 })}>{item.read ? '标为未读' : '标记已读'}</button><a href={item.url} target="_blank" rel="noreferrer">原始来源 <Link2 size={14} /></a></footer><small>{item.sourceNote}</small></article>)}{!visible.length && <Empty title="当前分类没有内容" text="切换分类，或配置并启用内容来源后刷新。" />}</section></div>{reader && <div className="reader-drawer" role="dialog" aria-modal="true"><article><header><div><span>{reader.source} · {reader.category}</span><h1>{reader.title}</h1><small>{reader.publishedAt} · {reader.level} · {reader.copyrightStatus}</small></div><button onClick={() => { setReaderId(''); setCustomExcerpt('') }}><X size={18} /></button></header><p className="reader-summary">{reader.summary}</p><div className="reader-keywords">{reader.keywords.map((item) => <span key={item}>{item}</span>)}</div>{reader.paragraphs.length ? <div className="reader-body">{reader.paragraphs.map((paragraph) => <section key={paragraph.id}><span>¶{paragraph.order}</span><p>{paragraph.text}</p>{paragraph.translatable && <button onClick={() => practice(reader, paragraph.text, paragraph.id)}>设为练习片段</button>}</section>)}</div> : <div className="copyright-boundary"><ShieldCheck size={18} /><p>此记录只保存标题、来源和平台摘要。请打开原始来源，选择你有权用于学习的短片段，再粘贴到下方。</p></div>}<label className="excerpt-box">自选练习片段<textarea value={customExcerpt} onChange={(event) => setCustomExcerpt(event.target.value)} placeholder="粘贴一个有权使用的段落或句子；平台会记录原始链接。" /></label><footer><a className="v2-btn subtle" href={reader.url} target="_blank" rel="noreferrer">打开原始来源 <Link2 size={14} /></a><button className="v2-btn primary" disabled={!customExcerpt.trim()} onClick={() => practice(reader, customExcerpt, 'user-selected')}>加入翻译练习 <ArrowRight size={14} /></button></footer></article></div>}</div>
}
...
function ReviewCenter({ cards, records, onWorkspace, onToast }: { cards: PhraseCard[]; records: Workspace['reviewRecords']; onWorkspace: (fn: (current: Workspace) => Workspace) => void; onToast: (message: string) => void }) {
  ...
  const cloze = active?.target ? active.target.replace(active.target.match(/[\p{L}\p{Script=Han}]{2,}/u)?.[0] ?? active.target, '______') : ''
  const contrast = cards.find((card) => card.id !== active?.id && card.domain === active?.domain)
  const prompt = !active ? '' : mode === 'target-source' ? active.target : mode === 'cloze' ? `${active.source}\n${cloze}` : active.source
  const answer = !active ? '' : mode === 'target-source' ? active.source : active.target
  return <div className="v2-stack"><PageHead eyebrow="REVIEW QUEUE" title="复习中心" text="五种模式使用不同的回忆任务，并把每次结果写入可解释的复习记录。" /><section className="review-stats"><div><strong>{due.length}</strong><span>今日到期</span></div><div><strong>{cards.filter((card) => card.type === 'term').length}</strong><span>术语卡</span></div><div><strong>{cards.filter((card) => card.type === 'phrase').length}</strong><span>表达卡</span></div><div><strong>{records.length}</strong><span>累计复习</span></div></section><div className="review-modes">{([['source-target','原文→译文'],['target-source','译文→原文'],['cloze','完形填空'],['contrast','相似辨析'],['retranslate','重新翻译']] as const).map(([value,label]) => <button key={value} className={mode === value ? 'active' : ''} onClick={() => { setMode(value); setRevealed(false); setAttempt('') }}>{label}</button>)}</div>{active ? <section className="review-stage"><aside>{cards.map((card) => <button key={card.id} className={card.id === active.id ? 'active' : ''} onClick={() => { setActiveId(card.id); setRevealed(false); setAttempt('') }}><span>{card.type}</span><strong>{card.source}</strong><small>掌握度 {card.mastery}/5</small></button>)}</aside><article className="review-card"><span>{active.domain} · {active.type} · {mode}</span><small>{active.context}</small><h2 className="review-prompt">{prompt}</h2>{mode === 'contrast' && <div className="contrast-pair"><div><span>表达 A</span><strong>{active.target}</strong></div><div><span>表达 B</span><strong>{contrast?.target || '暂无同领域对照表达'}</strong></div><p>先说明两种表达的语境和功能差异，再查看参考说明。</p></div>}{(mode === 'retranslate' || mode === 'cloze') && <textarea className="review-attempt" value={attempt} onChange={(event) => setAttempt(event.target.value)} placeholder={mode === 'retranslate' ? '重新完成译文后再查看参考答案……' : '填写空缺表达……'} />}<div className={revealed ? 'review-answer revealed' : 'review-answer'}><span>参考答案</span><strong>{answer}</strong><p>{mode === 'contrast' && contrast ? `对照：${contrast.target}。` : ''}{active.explanation || active.usage}</p>{attempt && <small>本次作答：{attempt}</small>}</div>{!revealed ? <button className="v2-btn primary" onClick={() => setRevealed(true)}><Eye size={16} />显示答案</button> : <div className="review-buttons"><button onClick={() => review('forgot')}>未掌握</button><button onClick={() => review('fuzzy')}>模糊</button><button onClick={() => review('mastered')}>掌握</button></div>}<footer>{active.sourceRef}</footer></article></section> : <Empty title="复习队列为空" text="从快速翻译或项目中加入表达后，会在这里安排复习。" />}</div>
}
...
function LanguageAssets({ workspace, onWorkspace, onToast }: { workspace: Workspace; onWorkspace: (fn: (current: Workspace) => Workspace) => void; onToast: (message: string) => void }) {
  ...
  const [query, setQuery] = useState('')
  const [editing, setEditing] = useState<{ tab: typeof tab; id: string } | null>(null)
  ...
  const references = workspace.personalReferences.filter((item) => `${item.title}${item.source}${item.summary}${item.tags.join('')}`.toLowerCase().includes(query.toLowerCase()))
  const openEditor = (kind: typeof tab, item?: TranslationUnit | TermEntry | StyleGuide | PersonalReference) => {
    setEditing({ tab: kind, id: item?.id ?? '' })
    if (kind === 'tm') { const value = item as TranslationUnit | undefined; setDraft({ source: value?.source ?? '', target: value?.target ?? '', domain: value?.domain ?? '通用', quality: value?.quality ?? 'reviewed' }) }
    if (kind === 'terms') { const value = item as TermEntry | undefined; setDraft({ source: value?.source ?? '', target: value?.target ?? '', definition: value?.definition ?? '', domain: value?.domain ?? '通用', partOfSpeech: value?.partOfSpeech ?? '', allowed: value?.allowed.join('，') ?? '', forbidden: value?.forbidden.join('，') ?? '', example: value?.example ?? '', sourceRef: value?.sourceRef ?? '', status: value?.status ?? 'candidate' }) }
    if (kind === 'styles') { const value = item as StyleGuide | undefined; setDraft({ name: value?.name ?? '', audience: value?.audience ?? '', formality: value?.formality ?? '', punctuation: value?.punctuation ?? '', dates: value?.dates ?? '', names: value?.names ?? '', preferred: value?.preferred.join('，') ?? '', forbidden: value?.forbidden.join('，') ?? '' }) }
    if (kind === 'references') { const value = item as PersonalReference | undefined; setDraft({ title: value?.title ?? '', source: value?.source ?? '', url: value?.url ?? '', summary: value?.summary ?? '', tags: value?.tags.join('，') ?? '' }) }
  }
  const list = (value: string) => value.split(/[，,]/).map((item) => item.trim()).filter(Boolean)
  const saveAsset = () => {
    if ((editing?.tab === 'tm' || editing?.tab === 'terms') && (!draft.source?.trim() || !draft.target?.trim())) { onToast('请填写源文和译文'); return }
    if (editing?.tab === 'styles' && !draft.name?.trim()) { onToast('请填写风格指南名称'); return }
    if (editing?.tab === 'references' && !draft.title?.trim()) { onToast('请填写资料标题'); return }
    const now = new Date().toISOString(); const id = editing?.id || createId(editing?.tab ?? 'asset')
    onWorkspace((current) => {
      if (editing?.tab === 'tm') { const item: TranslationUnit = { id, source: draft.source, target: draft.target, sourceLanguage: '英语（en）', targetLanguage: '简体中文（zh-CN）', domain: draft.domain, quality: draft.quality as TranslationUnit['quality'], useCount: current.translationMemory.find((value) => value.id === id)?.useCount ?? 0, createdAt: current.translationMemory.find((value) => value.id === id)?.createdAt ?? now, updatedAt: now }; return { ...current, translationMemory: [item, ...current.translationMemory.filter((value) => value.id !== id)] } }
      if (editing?.tab === 'terms') { const duplicate = current.terms.find((value) => value.id !== id && value.source.trim().toLowerCase() === draft.source.trim().toLowerCase() && value.domain === draft.domain); if (duplicate) { onToast('同一领域已存在相同源语术语'); return current } const item: TermEntry = { id, source: draft.source, target: draft.target, definition: draft.definition, domain: draft.domain, partOfSpeech: draft.partOfSpeech, allowed: list(draft.allowed), forbidden: list(draft.forbidden), example: draft.example, sourceRef: draft.sourceRef, status: draft.status as TermEntry['status'] }; return { ...current, terms: [item, ...current.terms.filter((value) => value.id !== id)] } }
      if (editing?.tab === 'styles') { const item: StyleGuide = { id, name: draft.name, audience: draft.audience, formality: draft.formality, punctuation: draft.punctuation, dates: draft.dates, names: draft.names, preferred: list(draft.preferred), forbidden: list(draft.forbidden) }; return { ...current, styleGuides: [item, ...current.styleGuides.filter((value) => value.id !== id)] } }
      const item: PersonalReference = { id, title: draft.title, source: draft.source, url: draft.url, summary: draft.summary, tags: list(draft.tags), createdAt: current.personalReferences.find((value) => value.id === id)?.createdAt ?? now, updatedAt: now }; return { ...current, personalReferences: [item, ...current.personalReferences.filter((value) => value.id !== id)] }
    }); setEditing(null); onToast(editing?.id ? '语言资产已更新' : '语言资产已新增')
  }
  const remove = (kind: typeof tab, id: string) => { if (!window.confirm('确定删除这条语言资产吗？此操作无法撤销。')) return; onWorkspace((current) => ({ ...current, translationMemory: kind === 'tm' ? current.translationMemory.filter((item) => item.id !== id) : current.translationMemory, terms: kind === 'terms' ? current.terms.filter((item) => item.id !== id) : current.terms, styleGuides: kind === 'styles' ? current.styleGuides.filter((item) => item.id !== id) : current.styleGuides, personalReferences: kind === 'references' ? current.personalReferences.filter((item) => item.id !== id) : current.personalReferences })); onToast('语言资产已删除') }
  const exportActive = () => downloadText(`yixue-${tab}.json`, JSON.stringify(tab === 'tm' ? workspace.translationMemory : tab === 'terms' ? workspace.terms : tab === 'styles' ? workspace.styleGuides : workspace.personalReferences, null, 2), 'application/json;charset=utf-8')
  return <div className="v2-stack"><PageHead eyebrow="LANGUAGE ASSETS" title="语言资产" text="把确认过的译法、术语和风格规则放在AI建议之前，并保留来源与审核状态。" actions={<><button className="v2-btn subtle" onClick={exportActive}><Download size={16} />导出当前库</button><button className="v2-btn subtle" onClick={() => exportWorkspace(workspace)}><Database size={16} />导出工作区</button></>} /><div className="asset-tabs">{([['tm','翻译记忆'],['terms','术语库'],['styles','风格指南'],['references','个人资料']] as const).map(([value,label]) => <button key={value} className={tab === value ? 'active' : ''} onClick={() => setTab(value)}>{label}<span>{value === 'tm' ? workspace.translationMemory.length : value === 'terms' ? workspace.terms.length : value === 'styles' ? workspace.styleGuides.length : workspace.personalReferences.length}</span></button>)}</div><section className="asset-workspace"><header><div><Search size={15} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="搜索源文、译文、术语或来源……" /></div><button onClick={() => openEditor(tab)}><Plus size={15} />新增</button></header>
    {tab === 'tm' && <div className="asset-table"><div className="asset-head"><span>源文</span><span>译文</span><span>领域 / 质量</span><span>操作</span></div>{tm.map((unit) => <div className="asset-row" key={unit.id}><p>{unit.source}</p><p>{unit.target}</p><span>{unit.domain}<small>{unit.quality} · 使用{unit.useCount}次</small></span><div className="row-actions"><button onClick={() => openEditor('tm', unit)}>编辑</button><button onClick={() => remove('tm', unit.id)}>删除</button></div></div>)}</div>}
    {tab === 'terms' && <div className="term-assets">{terms.map((term) => <article key={term.id}><header><span>{term.domain}</span><i>{term.status}</i></header><h2>{term.source}</h2><strong>{term.target || '译名待核验'}</strong><p>{term.definition || '尚未填写定义。'}</p><dl><div><dt>允许译法</dt><dd>{term.allowed.join('、') || '—'}</dd></div><div><dt>禁用译法</dt><dd>{term.forbidden.join('、') || '—'}</dd></div><div><dt>来源</dt><dd>{term.sourceRef || '待补充'}</dd></div></dl><div className="row-actions">{term.status === 'candidate' && <><button onClick={() => onWorkspace((current) => ({ ...current, terms: current.terms.map((item) => item.id === term.id ? { ...item, status: 'approved' } : item) }))}>批准</button><button onClick={() => onWorkspace((current) => ({ ...current, terms: current.terms.map((item) => item.id === term.id ? { ...item, status: 'rejected' } : item) }))}>拒绝</button></>}<button onClick={() => openEditor('terms', term)}>编辑</button><button onClick={() => remove('terms', term.id)}>删除</button></div></article>)}</div>}
    {tab === 'styles' && <div className="style-guides">{workspace.styleGuides.filter((guide) => `${guide.name}${guide.audience}`.toLowerCase().includes(query.toLowerCase())).map((guide) => <article key={guide.id}><span>STYLE GUIDE</span><h2>{guide.name}</h2><p>{guide.audience} · {guide.formality}</p><dl><div><dt>标点</dt><dd>{guide.punctuation}</dd></div><div><dt>日期</dt><dd>{guide.dates}</dd></div><div><dt>专名</dt><dd>{guide.names}</dd></div><div><dt>推荐</dt><dd>{guide.preferred.join('、')}</dd></div><div><dt>禁用</dt><dd>{guide.forbidden.join('、')}</dd></div></dl><div className="row-actions"><button onClick={() => openEditor('styles', guide)}>编辑</button><button onClick={() => remove('styles', guide.id)}>删除</button></div></article>)}</div>}
    {tab === 'references' && <div className="reference-list">{references.map((item) => <article key={item.id}><span>{item.source || '个人资料'}</span><h2>{item.title}</h2><p>{item.summary}</p><div>{item.tags.map((tag) => <i key={tag}>{tag}</i>)}</div><footer>{item.url && <a href={item.url} target="_blank" rel="noreferrer">查看来源</a>}<button onClick={() => openEditor('references', item)}>编辑</button><button onClick={() => remove('references', item.id)}>删除</button></footer></article>)}{!references.length && <Empty title="还没有个人参考资料" text="新增标题、来源、链接与摘要，可用于保存 Obsidian 文件索引或权威资料入口。" />}</div>}
  </section>{editing && <div className="asset-modal" role="dialog" aria-modal="true"><form onSubmit={(event) => { event.preventDefault(); saveAsset() }}><header><div><span>{editing.id ? '编辑' : '新增'}</span><h2>{editing.tab === 'tm' ? '翻译记忆' : editing.tab === 'terms' ? '术语' : editing.tab === 'styles' ? '风格指南' : '个人资料'}</h2></div><button type="button" onClick={() => setEditing(null)}><X size={17} /></button></header><div className="asset-form">{Object.entries(draft).map(([key,value]) => key === 'quality' ? <label key={key}>质量状态<select value={value} onChange={(event) => setDraft({ ...draft, [key]: event.target.value })}><option value="draft">草稿</option><option value="reviewed">已复核</option><option value="approved">已批准</option></select></label> : key === 'status' ? <label key={key}>审核状态<select value={value} onChange={(event) => setDraft({ ...draft, [key]: event.target.value })}><option value="candidate">候选</option><option value="approved">已批准</option><option value="rejected">已拒绝</option></select></label> : <label key={key} className={['definition','summary','example'].includes(key) ? 'wide' : ''}>{assetFieldLabel(key)}{['definition','summary','example'].includes(key) ? <textarea value={value} onChange={(event) => setDraft({ ...draft, [key]: event.target.value })} /> : <input type={key === 'url' ? 'url' : 'text'} value={value} onChange={(event) => setDraft({ ...draft, [key]: event.target.value })} />}</label>)}</div><footer><button type="button" onClick={() => setEditing(null)}>取消</button><button className="v2-btn primary" type="submit"><Save size={14} />保存</button></footer></form></div>}</div>
}

function SkillCenter({ workspace, onWorkspace, onToast }: { workspace: Workspace; onWorkspace: (fn: (current: Workspace) => Workspace) => void; onToast: (message: string) => void }) {
  const skills = workspace.skills
  const [activeId, setActiveId] = useState(skills[0]?.id ?? '')
  const [showLogs, setShowLogs] = useState(false)
  const skill = skills.find((item) => item.id === activeId) ?? skills[0]
  const runSkill = () => {
    if (!skill?.enabled) { onToast('此Skill尚未启用或尚未实现'); return }
    const project = workspace.projects.find((item) => item.status === 'active') ?? workspace.projects[0]
    const segments = project ? projectSegments(project) : []
    let outputSummary = ''; let success = true; let error = ''
    if (!project) { success = false; error = '没有可处理的翻译项目' }
    else if (skill.id === 'builtin.phrase-extractor') {
      const candidates = segments.filter((segment) => segment.status === 'confirmed' && segment.target.trim() && !workspace.phraseCards.some((card) => card.source === segment.source))
      const cards: PhraseCard[] = candidates.map((segment) => ({ id: createId('card'), type: 'phrase', source: segment.source, target: segment.target, context: project.name, usage: '来自用户确认译文，等待复习验证。', domain: project.domain, sourceRef: `${project.name} · 片段${segment.order}`, errorRecord: '', explanation: '由内置提取流程生成，未声称为权威译法。', tags: [project.domain], mastery: 0, nextReviewAt: new Date().toISOString(), createdAt: new Date().toISOString() }))
      onWorkspace((current) => ({ ...current, phraseCards: [...cards, ...current.phraseCards] })); outputSummary = `生成 ${cards.length} 张候选表达卡`
    } else if (['builtin.terminology-consistency', 'builtin.mt-tone-check', 'builtin.quality-analysis'].includes(skill.id)) {
      const all = inspectProject(segments, workspace.terms)
      const issues = skill.id === 'builtin.terminology-consistency' ? all.filter((item) => item.type.includes('术语')) : skill.id === 'builtin.mt-tone-check' ? all.filter((item) => item.type.includes('机器翻译腔') || item.type.includes('表达')) : all
      const ids = new Set(segments.map((segment) => segment.id)); onWorkspace((current) => ({ ...current, qualityIssues: [...current.qualityIssues.filter((item) => !ids.has(item.segmentId)), ...issues] })); outputSummary = `生成 ${issues.length} 条待人工复核问题`
    } else { success = false; error = '此Skill需要安全代理或内容授权，当前版本尚未实现' }
    const log = { id: createId('skill-log'), skillId: skill.id, skillName: skill.name, inputType: project ? 'project' : 'none', inputIds: project ? [project.id] : [], outputSummary, success, error, executedAt: new Date().toISOString() }
    onWorkspace((current) => ({ ...current, skillExecutionLogs: [log, ...current.skillExecutionLogs] })); onToast(success ? `${skill.name}已执行：${outputSummary}` : `${skill.name}未执行：${error}`)
  }
  return <div className="v2-stack"><PageHead eyebrow="CONTROLLED CAPABILITIES" title="Skill 中心" text="Skill 是受控工作流模块；每次执行都记录输入对象、输出摘要和失败原因。" actions={<button className="v2-btn subtle" onClick={() => setShowLogs(!showLogs)}><History size={16} />{showLogs ? '返回Skill' : `执行日志 ${workspace.skillExecutionLogs.length}`}</button>} />{showLogs ? <section className="execution-log">{workspace.skillExecutionLogs.map((log) => <article key={log.id} className={log.success ? 'success' : 'failed'}><span>{log.success ? <CheckCircle2 size={15} /> : <AlertTriangle size={15} />}</span><div><strong>{log.skillName}</strong><small>{dateTime(log.executedAt)} · {log.inputType} · {log.inputIds.join('、') || '无输入'}</small><p>{log.success ? log.outputSummary : log.error}</p></div></article>)}{!workspace.skillExecutionLogs.length && <Empty title="还没有执行记录" text="选择一个已实现并启用的Skill运行后，结果会保存在这里。" />}</section> : <div className="skill-layout"><aside>{skills.map((item) => <button key={item.id} className={item.id === skill?.id ? 'active' : ''} onClick={() => setActiveId(item.id)}><span className={item.enabled ? 'enabled' : ''}>{item.enabled ? <Check size={13} /> : <Circle size={13} />}</span><div><strong>{item.name}</strong><small>{item.category} · v{item.version}</small></div></button>)}</aside>{skill && <section className="skill-detail"><header><div><span>BUILT-IN SKILL</span><h1>{skill.name}</h1><p>{skill.description}</p></div><button className={skill.enabled ? 'skill-toggle on' : 'skill-toggle'} onClick={() => onWorkspace((current) => ({ ...current, skills: current.skills.map((item) => item.id === skill.id ? { ...item, enabled: !item.enabled } : item) }))}><i />{skill.enabled ? '已启用' : '已停用'}</button></header><div className="skill-meta"><div><span>作者与来源</span><strong>译学工作台内置</strong></div><div><span>版本</span><strong>{skill.version}</strong></div><div><span>更新时间</span><strong>{dateTime(skill.updatedAt)}</strong></div><div><span>执行类型</span><strong>{skill.entryType}</strong></div></div><h3>权限声明</h3><div className="permission-list">{skill.permissions.map((permission) => <span key={permission}><ShieldCheck size={14} />{permission}</span>)}</div><div className="io-grid"><div><span>接受输入</span>{skill.acceptedInput.map((item) => <strong key={item}>{item}</strong>)}</div><ArrowRight size={18} /><div><span>产生输出</span>{skill.producedOutput.map((item) => <strong key={item}>{item}</strong>)}</div></div><div className="boundary-box"><ShieldCheck size={18} /><p>{skill.enabled ? '执行结果只写入对应候选区或问题列表，不会直接修改译文。' : '此Skill当前停用；尚未实现的能力保持停用并明确说明依赖。'}</p></div><button className="v2-btn primary skill-run" disabled={!skill.enabled} onClick={runSkill}><Play size={15} />运行当前Skill</button></section>}</div>}</div>
}
...
const assetFieldLabel = (key: string) => ({ source: '源文／来源', target: '译文', domain: '领域', definition: '定义', partOfSpeech: '词性', allowed: '允许译法（逗号分隔）', forbidden: '禁用译法（逗号分隔）', example: '例句', sourceRef: '来源说明', name: '名称', audience: '目标读者', formality: '正式程度', punctuation: '标点规范', dates: '日期与数字规范', names: '人名地名处理', preferred: '推荐表达（逗号分隔）', title: '标题', url: '链接', summary: '摘要', tags: '标签（逗号分隔）' }[key] ?? key)
