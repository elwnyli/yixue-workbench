...
import {
  ...
  RefreshCcw, RotateCcw, Save, Search, Settings, ShieldCheck, SlidersHorizontal, Sparkles, Sun, Upload,
  WandSparkles, Wifi, WifiOff, X, Zap,
} from 'lucide-react'
import { cloneSeed, createId } from './data'
...
import { downloadText, exportWorkspace, loadWorkspace, saveWorkspace } from './storage'
import type {
...
export default function AppV2() {
  ...
  useEffect(() => { void loadWorkspace().then((state) => { setWorkspace(state); setQuick(state.quickSessions[0] ?? emptyQuick()); setSaveState('saved') }) }, [])
    ...
    <div className="v2-shell">
      ...
      <main className="v2-main">
        ...
        <div className="v2-page">
          ...
          {activeView === 'quick' && <QuickTranslate session={quick} workspace={workspace} onChange={setQuick} onWorkspace={patch} onSave={saveQuickSession} onToast={setToast} onSaveAsProject={(source, target) => { const file = parsePastedText('快速翻译转项目', source); file.segments[0].target = target; file.segments[0].initialTarget = quick.studentDraft; file.segments[0].status = target ? 'needs-confirmation' : 'untranslated'; createProject(buildProject('快速翻译项目', [file], workspace)) }} />}
...
function QuickTranslate({ session, workspace, onChange, onWorkspace, onSave, onToast, onSaveAsProject }: { session: QuickSession; workspace: Workspace; onChange: (session: QuickSession) => void; onWorkspace: (fn: (current: Workspace) => Workspace) => void; onSave: () => void; onToast: (message: string) => void; onSaveAsProject: (source: string, target: string) => void }) {
  ...
  const issues = inspectSegment({ id: session.id, order: 1, source: session.source, target: session.finalTranslation || session.studentDraft, initialTarget: session.studentDraft, status: 'translated', origin: 'human', note: '', revisions: [] }, workspace.terms)
  const run = async () => {
...
function ProjectCreator({ workspace, onCancel, onCreate, onToast }: { workspace: Workspace; onCancel: () => void; onCreate: (project: Project) => void; onToast: (message: string) => void }) {
  ...
  const [draft, setDraft] = useState({ name: '', description: '', sourceLanguage: '英语（en）', targetLanguage: '简体中文（zh-CN）', domain: '教育', textType: '政策文本', audience: '高校学生', style: '正式、自然', deadline: '', aiPretranslate: false, mtToneCheck: true, strictTerminology: true })
  ...
  return <div className="v2-stack"><PageHead eyebrow="NEW PROJECT" title="创建翻译项目" text="文件先在浏览器中解析；只有启用AI处理时，相关片段才会发送到配置的服务。" actions={<button className="v2-btn subtle" onClick={onCancel}><X size={16} />取消</button>} /><div className="create-steps">{['基本信息','导入文件','解析预览','资源与AI','确认'].map((label,index) => <button key={label} className={step === index + 1 ? 'active' : step > index + 1 ? 'done' : ''} onClick={() => setStep(index + 1)}><span>{step > index + 1 ? <Check size={13} /> : index + 1}</span>{label}</button>)}</div><section className="creator-layout"><div className="creator-main">
    ...
    {step === 4 && <div className="form-sheet"><h2>绑定资源与能力</h2><div className="resource-select"><label>翻译记忆库<select><option>个人翻译记忆（{workspace.translationMemory.length}条）</option></select></label><label>术语库<select><option>个人术语库（{workspace.terms.length}条）</option></select></label><label>风格指南<select><option>{workspace.styleGuides[0]?.name}</option></select></label></div><div className="check-options"><button className={draft.aiPretranslate ? 'on' : ''} onClick={() => setDraft({ ...draft, aiPretranslate: !draft.aiPretranslate })}><span>{draft.aiPretranslate ? <Check /> : <Circle />}</span><div><strong>AI预翻译</strong><small>创建后仍需确认发送范围</small></div></button><button className={draft.mtToneCheck ? 'on' : ''} onClick={() => setDraft({ ...draft, mtToneCheck: !draft.mtToneCheck })}><span>{draft.mtToneCheck ? <Check /> : <Circle />}</span><div><strong>机器翻译腔检测</strong><small>AI判断需要人工复核</small></div></button><button className={draft.strictTerminology ? 'on' : ''} onClick={() => setDraft({ ...draft, strictTerminology: !draft.strictTerminology })}><span>{draft.strictTerminology ? <Check /> : <Circle />}</span><div><strong>严格术语检查</strong><small>批准术语未使用时产生预警</small></div></button></div><h3>启用的内置Skill</h3><div className="skill-chips">{workspace.skills.filter((item) => item.enabled).map((item) => <span key={item.id}>{item.name}</span>)}</div></div>}
...
function ProjectEditor({ project, workspace, onBack, onWorkspace, onToast }: { project: Project; workspace: Workspace; onBack: () => void; onWorkspace: (fn: (current: Workspace) => Workspace) => void; onToast: (message: string) => void }) {
  ...
  const allVisible = filter === 'all' ? file?.segments ?? [] : file?.segments.filter((item) => item.status === filter) ?? []
  ...
  return <div className="editor-page"><header className="editor-toolbar"><button className="icon-button" onClick={onBack}>←</button><div><strong>{project.name}</strong><small>{file.name}</small></div><div className="editor-progress"><span>{projectProgress(project)}%</span><i><b style={{ width: `${projectProgress(project)}%` }} /></i></div><button><Search size={15} />搜索</button><button onClick={runQuality}><ShieldCheck size={15} />质量检查</button><button onClick={exportPure}><Download size={15} />纯译文</button><button onClick={exportBilingual}><FileCheck2 size={15} />双语文档</button><span className="saved-label"><Check size={13} />自动保存</span><button className="icon-button" onClick={() => setRightOpen(!rightOpen)}>{rightOpen ? <PanelRightClose size={17} /> : <PanelRightOpen size={17} />}</button></header>
    <div className={rightOpen ? 'editor-shell' : 'editor-shell right-closed'}><aside className="file-rail"><span>项目文件</span>{project.files.map((item) => <button key={item.id} className={item.id === file.id ? 'active' : ''} onClick={() => { setActiveFileId(item.id); setActiveSegmentId(item.segments[0]?.id ?? '') }}><FileText size={15} /><div><strong>{item.name}</strong><small>{item.segments.length}个片段</small></div></button>)}<span>片段状态</span>{([['all','全部'],['untranslated','未翻译'],['needs-confirmation','待确认'],['confirmed','已确认'],['issue','有问题']] as const).map(([value,label]) => <button key={value} className={filter === value ? 'active' : ''} onClick={() => setFilter(value)}><Circle size={12} /><div><strong>{label}</strong><small>{value === 'all' ? file.segments.length : file.segments.filter((item) => item.status === value).length}</small></div></button>)}</aside>
      <section className="segment-list">{allVisible.map((segment,index) => <article key={segment.id} className={segment.id === active.id ? `segment-row active status-${segment.status}` : `segment-row status-${segment.status}`} onClick={() => setActiveSegmentId(segment.id)}><header><span>#{String(segment.order).padStart(3,'0')}</span><i>{segment.status}</i><b>{originLabel(segment.origin)}</b><small>{segment.target.length}字</small></header><div className="segment-source"><span>SOURCE</span><p>{segment.source}</p></div><div className="segment-target"><span>TARGET</span><textarea value={segment.target} onClick={(event) => event.stopPropagation()} onChange={(event) => updateSegment(segment.id, { target: event.target.value, initialTarget: segment.initialTarget || event.target.value, status: 'translating', origin: 'human' })} placeholder="输入目标译文……" /></div><footer><button onClick={(event) => { event.stopPropagation(); split(segment) }}>拆分</button><button onClick={(event) => { event.stopPropagation(); mergeNext(segment) }}>与下段合并</button><button onClick={(event) => { event.stopPropagation(); updateSegment(segment.id, { status: 'issue' }); onToast('已标记为有问题') }}><AlertTriangle size={13} />标记问题</button><button className="confirm-button" onClick={(event) => { event.stopPropagation(); confirm(segment) }}><Check size={14} />确认译文</button></footer>{index === allVisible.length - 1 && <small className="list-end">已到当前文件末尾</small>}</article>)}</section>
      {rightOpen && <aside className="editor-assist"><nav>{([['ai','AI建议'],['tm','翻译记忆'],['terms','术语库'],['context','上下文'],['quality','质量问题'],['history','修改记录'],['notes','批注笔记']] as const).map(([value,label]) => <button key={value} className={rightTab === value ? 'active' : ''} onClick={() => setRightTab(value)}>{label}</button>)}</nav><div>
        {rightTab === 'ai' && <><div className="ai-boundary"><Bot size={16} /><span><strong>{workspace.settings.provider === 'demo' ? '演示分析' : workspace.settings.model}</strong>建议不会自动覆盖当前译文。</span></div>{inspectSegment(active, workspace.terms).slice(0,3).map((issue) => <div className="assist-note" key={issue.id}><span>{issue.type}</span><p>{issue.message}</p><small>{issue.suggestion}</small></div>)}</>}
        ...
        {rightTab === 'quality' && (projectIssues.length ? projectIssues.map((issue) => { const issueFile = project.files.find((item) => item.segments.some((segment) => segment.id === issue.segmentId)); const issueSegment = issueFile?.segments.find((segment) => segment.id === issue.segmentId); return <div className={`issue-row ${issue.severity}`} key={issue.id}><AlertTriangle size={15} /><div><strong>{issue.type}</strong><p>{issue.message}</p><small>{issueFile?.name} · 片段 {issueSegment?.order}</small><small>{issue.evidence}</small><button onClick={() => { if (issueFile) setActiveFileId(issueFile.id); setActiveSegmentId(issue.segmentId) }}>定位到片段</button></div></div> }) : <Empty title="暂无质量问题" text="点击顶部“质量检查”运行项目检查。" />)}
...
function SettingsPage({ workspace, onWorkspace, onToast }: { workspace: Workspace; onWorkspace: (fn: (current: Workspace) => Workspace) => void; onToast: (message: string) => void }) {
  ...
  const test = async () => {
    ...
    setTesting(true)
    try { const response = await fetch(settings.endpoint, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ action: 'test', apiKey }) }); onToast(response.ok ? '代理连接测试成功' : `连接失败：服务返回 ${response.status}`) } catch { onToast('连接失败，请检查端点、网络和跨域设置') } finally { setTesting(false); setApiKey('') }
  }
  return <div className="v2-stack"><PageHead eyebrow="WORKSPACE SETTINGS" title="设置" text="模型、外观与本地数据由你控制；API密钥不会写入工作区、日志或导出文件。" /><div className="settings-layout"><section><header><Bot size={20} /><div><h2>DeepSeek 连接</h2><p>正式调用通过安全代理；演示模式不会发送翻译文本。</p></div></header><div className="setting-modes"><button className={settings.provider === 'demo' ? 'active' : ''} onClick={() => patch({ provider: 'demo' })}><ShieldCheck size={18} /><strong>演示模式</strong><small>本地规则与固定示例</small></button><button className={settings.provider === 'deepseek-proxy' ? 'active' : ''} onClick={() => patch({ provider: 'deepseek-proxy' })}><Wifi size={18} /><strong>安全代理</strong><small>连接自有后端</small></button></div>{settings.provider === 'deepseek-proxy' && <div className="setting-fields"><label>代理端点<input value={settings.endpoint} onChange={(event) => patch({ endpoint: event.target.value })} placeholder="https://your-service.example/api/deepseek" /></label><label>模型<select value={settings.model} onChange={(event) => patch({ model: event.target.value })}><option>deepseek-chat</option><option>deepseek-reasoner</option></select></label><label>临时 API Key<div className="password-field"><input type={showKey ? 'text' : 'password'} value={apiKey} onChange={(event) => setApiKey(event.target.value)} placeholder="仅用于本次连接测试" /><button onClick={() => setShowKey(!showKey)}><Eye size={15} /></button></div><small>Key只保存在当前组件内存中，测试后立即清空。代理应改用服务器密钥或返回加密凭证引用。</small></label><button className="v2-btn primary" disabled={testing} onClick={() => void test()}>{testing ? <LoaderCircle className="spin" size={16} /> : <Wifi size={16} />}测试连接</button></div>}<div className="security-callout"><AlertTriangle size={17} /><p>不要把DeepSeek密钥写入GitHub Pages代码、localStorage、控制台日志或项目导出文件。</p></div></section>
      <section><header><SlidersHorizontal size={20} /><div><h2>请求参数</h2><p>控制单次长度和批量大小，避免大文本一次发送。</p></div></header><div className="setting-fields"><label>温度：{settings.temperature}<input type="range" min="0" max="1" step="0.1" value={settings.temperature} onChange={(event) => patch({ temperature: Number(event.target.value) })} /></label><label>单次字符上限<input type="number" value={settings.requestLimit} onChange={(event) => patch({ requestLimit: Number(event.target.value) })} /></label><label>批量片段数<input type="number" value={settings.batchSize} onChange={(event) => patch({ batchSize: Number(event.target.value) })} /></label></div></section>
      ...
      <section><header><Database size={20} /><div><h2>数据与备份</h2><p>V2项目主体存放在浏览器IndexedDB中。</p></div></header><div className="data-buttons"><button className="v2-btn subtle" onClick={() => exportWorkspace(workspace)}><Download size={16} />导出V2工作区</button><button className="v2-btn danger" onClick={() => { const next = cloneSeed(); onWorkspace(() => next); onToast('已恢复V2演示数据') }}><RotateCcw size={16} />恢复演示数据</button></div><p className="data-boundary">清除浏览器站点数据会删除本地项目。正式学生试用前还需确定身份、匿名化、保留期限和伦理路径。</p></section></div></div>
}
