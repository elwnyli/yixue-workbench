...
export const seedWorkspace: Workspace = {
  ...
  skills: [
    ...
    { id: 'builtin.quality-analysis', name: '翻译质量分析', version: '1.0.0', description: '运行可解释的本地质量规则并生成待人工处理的问题。', category: 'quality', permissions: ['read:project-segments', 'read:project-termbase', 'write:quality-issue'], acceptedInput: ['project'], producedOutput: ['quality-issue'], entryType: 'builtin', configurationSchema: {}, enabled: true, updatedAt: now },
    { id: 'builtin.cultural-concepts', name: '文化概念识别', version: '0.1.0', description: '需要安全代理与来源检索后才可运行，当前不输出无来源解释。', category: 'terminology', permissions: ['read:current-segment', 'use:model', 'use:source-proxy'], acceptedInput: ['segment'], producedOutput: ['cultural-note'], entryType: 'builtin', configurationSchema: {}, enabled: false, updatedAt: now },
    { id: 'builtin.news-excerpt', name: '新闻选段提取', version: '0.1.0', description: '需要内容同步代理和版权状态判断，当前由用户手动选择合法片段。', category: 'learning', permissions: ['read:content-item', 'write:practice-exercise'], acceptedInput: ['content-item'], producedOutput: ['practice-exercise'], entryType: 'builtin', configurationSchema: {}, enabled: false, updatedAt: now },
  ],
  skillExecutionLogs: [],
  aiRequestLogs: [],
  batchJobs: [],
