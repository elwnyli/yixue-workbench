...
- 双语编辑器：可续跑的AI批量预翻译、人工覆盖保护、历史版本、术语查询、翻译记忆、批注和导出
- 项目级质量检查：漏译、数字、原文残留、长度、术语和占位符
- 语言资产：翻译记忆、术语库、风格指南和个人资料的本地 CRUD
- 译闻与素材：来源注册、代理同步、分类阅读、收藏/已读和有版权边界的选段练习
- Skill中心：声明式权限、启停、内置检查执行和持久化执行日志，不执行任意代码
- 复习中心：原译、反向、完形、辨析和重新翻译五种任务模式
...
端点返回 `reference` 和 `feedback`。端点调用失败时，平台明确标注回退到演示反馈并写入失败日志，不会把演示结果冒充真实模型结果。
